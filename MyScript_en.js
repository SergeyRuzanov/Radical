...
function someFunc() {
    ...
    if (chil.trim() === '') {
        document.getElementById('d').innerHTML = '<span>Error!</span>Enter the number!';
        error[0].style.display = "block";
    } else {
        if (+round > 40) {
            document.getElementById('d').innerHTML = '<span>Error!</span>Output up to a maximum of 40 decimal places!';
            error[0].style.display = "block";
        } else if (Number.isInteger(+round) == false || +round < 0) {
            document.getElementById('d').innerHTML = '<span>Error!</span>Enter a positive integer to round!';
            error[0].style.display = "block";
        } else {
            if (isNaN(chil) == false) {
                if (+chil > 0) {
                    if (Number.isInteger(Math.sqrt(+chil))) {
                        document.getElementById('c').innerHTML = '<span>Answer:</span>' + '&plusmn;' + Math.sqrt(+chil);
                    } else {
                        document.getElementById('c').innerHTML = '<span>Answer:</span>' + '&plusmn;' + Math.sqrt(+chil).toFixed(round);
                    }
                    answer[0].style.display = "block";
                } else if (+chil < 0) {
                    chil = math.complex(chil);
                    document.getElementById('c').innerHTML = '<span>Answer:</span>' + '&plusmn;' + (Number.isInteger(math.sqrt(chil).im) ? (math.sqrt(chil).im == 1 ? 'i' : math.sqrt(chil).im + 'i') : math.sqrt(chil).im.toFixed(round) + 'i');
                    answer[0].style.display = "block";
                } else if (+chil === 0) {
                    document.getElementById('c').innerHTML = '<span>Answer:</span>' + Math.sqrt(+chil);
                    answer[0].style.display = "block";
                }
            } else {
                try {
                    if (chil.search(/sin/i) != -1 || chil.search(/cos/i) != -1 || chil.search(/tg/i) != -1) {
                        ...
                        if (!Number.isNaN(chil)) {
                            if (chil > 0) {
                                ...
                                document.getElementById('d').innerHTML = '<span>Error!</span>Invalid number entered!';
                                error[0].style.display = "block";
                            }
                        } else {
                            document.getElementById('d').innerHTML = '<span>Error!</span>Invalid number entered!';
                        ...
                        if (+chil > 0) {
                            ...
                            chil = math.complex(chil);
                            document.getElementById('c').innerHTML = '<span>Answer:</span>' + '&plusmn;' + Number.isInteger(math.sqrt(chil).im) ? math.sqrt(chil).im + 'i' : math.sqrt(chil).im.toFixed(round) + 'i';
                            answer[0].style.display = "block";
                        } else if (+chil === 0) {
                            document.getElementById('c').innerHTML = '<span>Answer:</span>' + Math.sqrt(+chil);
                            answer[0].style.display = "block";
